...
fn main() {
    let log_path = "test_data/Apache/Apache_2k.log";
    
    println!("Tango Log Parser - Apache Log Test");
    println!("===================================");
    println!("Parsing: {}\n", log_path);
    
    let file = File::open(log_path).expect("Failed to open Apache log file");
    ...
    for line in reader.lines() {
        ...
        *format_counts.entry(format_name).or_insert(0) += 1;
        ...
        if total <= 5 {
            ...
            println!("  Message:   {}", event.message.chars().take(80).collect::<String>());
